...
เมื่อ module WebAssembly export memory ของมัน JavaScript จะได้รับ `WebAssembly.Memory` object property `.buffer` ของมันคือ `ArrayBuffer` มาตรฐาน — ชนิดเดิมที่ใช้ทั่ว Web Platform ซึ่งหมายความว่าคุณสามารถวาง typed-array view ใดก็ได้ทับลงไปเพื่ออ่านและเขียน memory ได้อย่างมีประสิทธิภาพ
...
- สร้าง `Int32Array` หรือ `Float64Array` ที่ byte offset ซึ่งไม่ align กับขนาด element ของมัน (เช่น offset ที่ไม่ใช่ multiple ของ 4 สำหรับ `Int32Array`) ทำให้ constructor throw error
...
> 💡 Real World Example
>
> เครื่องมือแก้ไขภาพในเบราว์เซอร์สไตล์ Squoosh อ่าน raw pixel buffer เดียวกันผ่านทั้ง `Uint8ClampedArray` และ `Float32Array` view แล้วแต่ operation ที่กำลังทำ โดยไม่ต้อง copy ข้อมูลภาพไปมาระหว่าง view แต่ละแบบ
