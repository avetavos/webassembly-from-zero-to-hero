...
> 💡 ตัวอย่างจากของจริง
>
> **Emscripten** เรียก `wasm-opt` ของ Binaryen อัตโนมัติเมื่อ build ด้วย `-O2`/`-O3` ([เอกสาร Emscripten](https://emscripten.org/docs/optimizing/Optimizing-Code.html)) ส่วนเครื่องมือในบทนี้ (`wasm-objdump`, `twiggy`, `wasm2wat`) คือสิ่งที่หยิบมาใช้ทีหลัง เมื่อต้องรู้ให้แน่ชัดว่า binary ที่ optimize แล้วมีอะไรอยู่ข้างในบ้าง
