...
วิธีที่เร็วที่สุดในการอ่าน binary ใดก็ตามคือ `wasm2wat` มันสร้าง WAT text representation ขึ้นมาใหม่ รวมถึงชื่อฟังก์ชันหาก name section มีอยู่
...
`wasm-objdump -x` แสดงสรุปที่มีโครงสร้างของทุก section: type signatures, imports, exports และ function indices มันเร็วกว่า full disassembly เมื่อคุณต้องการแค่ public API ของ module
...
เบราว์เซอร์สมัยใหม่มาพร้อมกับการสนับสนุน WebAssembly debugging โดยตรงใน DevTools ใน Chrome และ Firefox คุณสามารถเปิด **Sources → wasm** เพื่อดู disassembly ของทุก module ที่ instantiate แล้ว ตั้ง breakpoints บนคำสั่งแต่ละตัว และตรวจสอบ local variables และ value stack ที่แต่ละ breakpoint
