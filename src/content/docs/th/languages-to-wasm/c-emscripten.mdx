...
หากคุณมีโค้ด C หรือ C++ ที่ทำงานได้บน Desktop อยู่แล้ว Emscripten มักจะสามารถคอมไพล์ไปยัง Wasm ได้โดยแก้ไขน้อยมาก เพราะจำลอง `printf`, `malloc`, File I/O และแม้แต่ OpenGL (ผ่าน WebGL) สำหรับโค้ดใหม่ ควรเลือก Rust หรือ AssemblyScript เพื่อ Output ที่กระชับกว่า
</Callout>
...
> 💡 ตัวอย่างจากของจริง
>
> Figma คอมไพล์ C++ Rendering Engine ของตัวเองด้วย Emscripten เพื่อรันบนเว็บแอป ทำให้ได้ Performance ระดับ Desktop โดยไม่ต้อง Rewrite Engine ทั้งหมดเป็นภาษาอื่น — บริษัทวัดผลได้ว่าเวลาโหลด **เร็วขึ้นมากกว่า 3 เท่า** หลังเปลี่ยนมาใช้ Wasm ([Figma engineering blog](https://www.figma.com/blog/webassembly-cut-figmas-load-time-by-3x/))

export const quizCEmscripten = [
  { q: "ทำไม Entry ใน EXPORTED_FUNCTIONS จึงต้องขึ้นต้นด้วย Underscore (เช่น '_hypotenuse')?", options: ["ทำเครื่องหมายฟังก์ชันว่าเป็น Private", "ชื่อ Symbol ของ C จะถูกเติม _ ตามธรรมเนียมของ Emscripten", "Underscore เปิดใช้งานการ Optimise SIMD", "ป้องกัน Name Mangling ใน C++"], answer: 1 },
