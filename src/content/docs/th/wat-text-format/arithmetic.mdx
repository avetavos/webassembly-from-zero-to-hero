...
ชนิดจำนวนเต็มของ WebAssembly (`i32`, `i64`) ก็แค่รูปแบบบิต — ไม่ได้พกพาเครื่องหมายในตัวเอง การหารและเศษเหลือมีอยู่สองตัวแปร: `div_s` / `rem_s` ตีความบิตเป็นจำนวนเต็มแบบมีเครื่องหมายในรูป two's-complement; `div_u` / `rem_u` ตีความเป็นแบบไม่มีเครื่องหมาย การเปรียบเทียบก็เป็นไปตามกฎเดียวกัน: `i32.lt_s` เทียบกับ `i32.lt_u` ชนิดเพียงอย่างเดียวไม่เคยบอกคุณว่าใช้การตีความแบบใด
</Callout>
...
> 💡 ตัวอย่างจากของจริง
>
> [ชุด test อย่างเป็นทางการของ WebAssembly spec](https://github.com/WebAssembly/spec/tree/main/test/core) — ซึ่งเครื่องมืออย่าง WABT นำไป mirror ไว้ให้ใช้งานสะดวก — มีไฟล์ `.wat` ที่เขียนมือจำนวนมากเพื่อยืนยันว่า opcode อย่าง `div_s`/`div_u` ทำงานตรงตาม spec ทุกกรณี รวมถึง edge case อย่างการหารด้วยศูนย์
...
export const quizArithmetic = [
  ...
  },
  {
    ...
    options: [
      "div_s เร็วกว่า",
      "div_s ปฏิบัติต่อตัวถูกดำเนินการเป็นแบบมีเครื่องหมาย; div_u ปฏิบัติต่อเป็นแบบไม่มีเครื่องหมาย",
