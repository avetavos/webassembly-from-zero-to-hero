...
|---|---|---|
| `instantiateStreaming` | เริ่มคอมไพล์ทันทีที่ bytes เริ่มไหลมาจาก network ไม่ต้องรอดาวน์โหลดครบ ทำให้ exports พร้อมใช้เร็วขึ้น | ต้องพึ่ง server ที่ตั้งค่า `Content-Type: application/wasm` ให้ถูกต้อง ไม่งั้น reject ทันที |
| `instantiate` แบบ buffer เต็มไฟล์ | เขียนง่าย ควบคุม flow ได้ตรงไปตรงมา ใช้ได้แม้ไม่ควบคุม MIME type ของ server | ต้องรอดาวน์โหลดครบก่อนเริ่มคอมไพล์ exports จึงพร้อมใช้ช้ากว่า |
...
> 💡 ตัวอย่างจากของจริง
>
> [Squoosh](https://github.com/GoogleChromeLabs/squoosh) เครื่องมือบีบอัดภาพของ Google คอมไพล์ codec หลายตัว (MozJPEG, WebP, AVIF ฯลฯ) เป็น Wasm แล้วเรียกใช้ exports เหล่านั้นตรงๆ จาก JS พร้อมอ่านผลลัพธ์ภาพผ่าน exported memory เพื่อให้ได้ throughput สูงสุด
...
export const quizExports = [
  ...
  { q: "เหตุใด Wasm จึงใช้ BigInt สำหรับ i64 แทนที่จะเป็น number?", options: ["BigInt ทำงานเร็วกว่า", "JavaScript number ไม่สามารถแทนค่า i64 ทั้งหมดได้อย่างแม่นยำ", "browser กำหนดให้ใช้เพื่อความปลอดภัย", "BigInt คือชนิดจำนวนเต็มเริ่มต้น"], answer: 1 },
];
